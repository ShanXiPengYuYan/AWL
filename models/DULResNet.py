...
class ResNetSE(nn.Module):
    def __init__(self, block, layers, num_filters, nOut, encoder_type='SAP', n_mels=40, specaug=None, **kwargs):
        ...
        # self.mu = nn.Linear(out_dim, nOut)
        #
        # self.logvar = nn.Linear(out_dim, nOut)
        self.mu = nn.Sequential(
            nn.Linear(out_dim, nOut),
            nn.BatchNorm1d(nOut)
        )
    ...
    def forward(self, x, aug=False):
        ...
        x = x.view(x.size()[0], -1)
        speaker_mu = self.mu(x)
        speaker_logvar = self.logvar(x)
        embedding = self._reparameterize(speaker_mu, speaker_logvar)
        if self.training:
            return (speaker_mu, speaker_logvar, embedding)
        else:
            return speaker_mu
